...
class CvHLinesDashed extends Canvas3D {
   ...
   private void buildLineSet() { 
      ...
      for (int j = 0; j < nFaces; j++) {
        ...
        // if (n > 2 && pol.getH() > 0) continue; 
               // backface culling disabled for dashed lines
   ...
   void dashedLine(Graphics g, float xA, float yA,
		           float xB, float yB, float dashLength) {
      float u1 = xB - xA, u2 = yB - yA, 
    		len = (float)Math.sqrt(u1 * u1 + u2 * u2);
	  int n = Math.round((len/dashLength + 1)/2);
   ...
   private void lineSegment(Graphics g, Point3D p, Point3D q,
	  ...
	  if (iStart == 0) 
	    dashedLine(g, pScr.x, pScr.y, qScr.x, qScr.y, 5);
